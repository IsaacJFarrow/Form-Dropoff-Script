(function () {
  ...
  function sendTrackRequest(data) {
    const params = new URLSearchParams({ ...data, sessionId });
    // GET with no-cors: no preflight, no CORS errors, works through Webflow Cloud proxy.
    fetch(`${SERVER_URL}/api/track?${params}`, {
      mode: "no-cors",
      keepalive: true,
    }).catch(() => {});
  }
  ...
  function observeFormVisibility() {
    ...
    const checkAndStartTracking = () => {
      if (
        window.getComputedStyle(formElement).display !== "none" &&
        !isTrackingActive
      ) {
  ...
  function observeFormSubmission() {
    ...
    const observer = new MutationObserver(() => {
      if (
        window.getComputedStyle(successElement).display !== "none" &&
        !submissionSent
      ) {
        submissionSent = true;

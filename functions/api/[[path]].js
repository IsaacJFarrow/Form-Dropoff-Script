import { UpstashRedis } from "./upstash.js";
 ...
 * @param {UpstashRedis} redisClient
 * @returns {Promise<Response>}
 */
async function handleTrackRequest(request, redisClient) {
  ...
  const key = `event:${eventData.timestamp}:${Math.random()
    .toString(36)
    .substr(2, 9)}`;
  await redisClient.put(key, JSON.stringify(eventData));
 ...
 * @returns {Promise<Response>}
 */
async function handleDataRequest(redisClient) {
  const now = Date.now();
  ...
  const list = await redisClient.list({ prefix: "event:" });
  const promises = list.keys.map((key) =>
    redisClient.get(key.name, { type: "json" })
  );
...
export async function onRequest(context) {
  ...
  try {
    ...
    // Get Upstash Redis credentials from environment variables
    const upstashUrl = env.UPSTASH_REDIS_REST_URL;
    const upstashToken = env.UPSTASH_REDIS_REST_TOKEN;

    if (!upstashUrl || !upstashToken) {
      throw new Error(
        "Upstash Redis credentials not configured. Please set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN in your Cloudflare Pages environment variables."
      );
    }

    // Initialize Upstash Redis client
    const redisClient = new UpstashRedis(upstashUrl, upstashToken);

    const path = url.pathname.replace("/api/", "");

    if (path === "track" && request.method === "POST") {
      response = await handleTrackRequest(request, redisClient);
    } else if (path === "data" && request.method === "GET") {
      response = await handleDataRequest(redisClient);
    ...
    response = new Response(
      JSON.stringify({ error: errorMessage, details: err.stack }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
